...
const NewestPosts = async () => {
    const {message, posts, redirectUrl, success} = await fetchPublishedPosts("", 1, 3);
                ...
                posts.map((post:Post) => {
                    ...
                    return (
                        <Link
                            ...
                            <p className="text-[13px] font-medium">{post.excerpt.length > 100 ? post.excerpt.slice(0, 100) + "..." : post.excerpt}</p>
